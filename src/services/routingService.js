...
async function handleConversationCommand(message, client, discordId, content) {
  const pauseMatch = content.match(/^pause\s+(\d+)$/i);
  const continueMatch = content.match(/^continue\s+(\d+)$/i);
  const endMatch = content.match(/^end\s+(\d+)$/i);
...
async function handlePause(message, client, discordId, chatId) {
  ...
  await message.channel.send(`**المحادثة #${chatId}** أوقفت مؤقتاً. يمكنك أنت فقط استئنافها بأمر \`continue ${chatId}\`.`);
